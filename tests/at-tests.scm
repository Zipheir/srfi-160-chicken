...
(test-group "@vector tests"
  ...
  (define-syntax test-equiv
    (syntax-rules ()
      ((test-equiv expect expr)
       (test expect (map inexact->exact (@vector->list expr))))
      ((test-equiv name expect expr)
       (test name expect (map inexact->exact (@vector->list expr))))))
  ...
  (test-group "@vector"
    ...
    (test-group "@vector/selectors"
      (test "ref" 1 (inexact->exact (@vector-ref (@vector 1 2 3) 0)))
    ...
    (test-group "@vector/iteration"
      ...
      (test "segment" (list (@vector 1 2 3) (@vector 4 5))
            (@vector-segment s5 3))
      (test "fold" -6 (inexact->exact (@vector-fold - 0 (@vector 1 2 3))))
      ;; (test "fold" '(((0 1 4) 2 5) 3 6)
      ;;       (@vector-fold list 0 (@vector 1 2 3) (@vector 4 5 6)))
      (test "fold-right" -6 (inexact->exact (@vector-fold-right - 0 (@vector 1 2 3))))
      ;; (test "fold-right" '(((0 3 6) 2 5) 1 4)
      ;;       (@vector-fold-right list 0 (@vector 1 2 3) (@vector 4 5 6)))
      (test-equiv "map" '(1 4 9 16 25) (@vector-map (cut expt <> 2) s5))
      (test-equiv "map" '(0 0 0 0 0) (@vector-map - s5 s5))
      (let ((v (@vector 1 2 3 4 5)))
        (@vector-map! (cut expt <> 2) v)
    ...
    (test-group "@vector/searching"
      ...
      (test "any" 4 (inexact->exact (@vector-any (lambda (x) (and (even? x) (* x 2))) s5)))
      (test-assert "not any" (not (@vector-any list? s5)))
      (test "any + 1" 2 (inexact->exact (@vector-any odd+1 s5)))
      (test-assert "every" (@vector-every number? s5))
      (test-assert "not every" (not (@vector-every odd? s5)))
      (test-assert "every + 1" (not (@vector-every odd+1 s5)))
      (test "multi-any" 10 (inexact->exact (@vector-any (lambda (x y) (and (even? x) (even? y) (+ x y)))
                                                        s5 (@vector 0 1 2 6 4))))
    ...
    (test-group "@vector/mutators"
      ...
      (let ((v (@vector 1 2 3 4 5 6 7 8)))
        ;; (display "unfold!")
        (@vector-unfold! (lambda (x) (values (* x 2) (* x 2)))
                         v 1 6 1)
        (test-equiv "vector-unfold!" '(1 2 4 8 16 32 7 8) v))
      (let ((v (@vector 1 2 3 4 5 6 7 8)))
        ;; (display "unfold-right!")
        (@vector-unfold-right! (lambda (x) (values (* x 2) (* x 2)))
                               v 1 6 1)
    ...
    (test-group "@vector/conversion"
      (test "@vector->list 1" '(1 2 3 4 5)
            (map inexact->exact (@vector->list s5)))
      (test "@vector->list 2" '(2 3 4 5)
            (map inexact->exact (@vector->list s5 1)))
      (test "@vector->list 3" '(2 3 4)
            (map inexact->exact (@vector->list s5 1 4)))
      (test "@vector->vector 1" #(1 2 3 4 5)
            (vector-map inexact->exact (@vector->vector s5)))
      (test "@vector->vector 2" #(2 3 4 5)
            (vector-map inexact->exact (@vector->vector s5 1)))
      (test "@vector->vector 3" #(2 3 4)
            (vector-map inexact->exact (@vector->vector s5 1 4)))
    ...
    (test-group "@vector/misc"
      ...
      ;;   (test "write-@vector" "#@(1 2 3 4 5)" (get-output-string port))
      ...
      (test "@vector-gen 0" 1 (inexact->exact (g)))
      (test "@vector-gen 1" 2 (inexact->exact (g)))
      (test "@vector-gen 2" 3 (inexact->exact (g)))
      (test "@vector-gen 3" 4 (inexact->exact (g)))
      (test "@vector-gen 4" 5 (inexact->exact (g)))
