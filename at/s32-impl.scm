...
(define (s32vector-copy* vec start end)
  (let ((v (make-s32vector (fx- end start))))
...
(define s32vector-copy!
  (case-lambda
    ((to at from)
     (move-memory! from to (s32vector-length from) 0 (fx* at 4)))
    ((to at from start)
     (move-memory! from to (s32vector-length from) (fx* start 4) (fx* at 4)))
    ...
    (let loop ((vecs vecs) (at 0))
      (unless (null? vecs)
        (let ((vec (car vecs)))
          ...
          (loop (cdr vecs) (fx+ at (s32vector-length vec)))))
    v)))

(define (len-sum vecs)
  (if (null? vecs)
    0
    (fx+ (s32vector-length (car vecs))
       (len-sum (cdr vecs)))))
...
(define (s32dyadic-vecs= vec1 start1 end1 vec2 start2 end2)
  (cond
    ...
    ((let ((elt1 (s32vector-ref vec1 start1))
      ...
      (= elt1 elt2))
     (s32dyadic-vecs= vec1 (fx+ start1 1) end1
...
(define (s32vector-take-right vec n)
  (let ((v (make-s32vector n))
    ...
    (s32vector-copy! v 0 vec (fx- len n) len)
    v))
  ...
  (let loop ((r '()) (i 0) (remain (s32vector-length vec)))
    (if (fx<= remain 0)
      ...
      (let ((size (min n remain)))
        (loop
          (cons (s32vector-copy vec i (fx+ i size)) r)
          (fx+ i size)
          (fx- remain size))))))
  ...
  (if (null? vecs)
    ...
    (let ((len (s32vector-length vec)))
      (let loop ((r knil) (i 0))
        (if (fx= i len)
          r
          (loop (kons r (s32vector-ref vec i)) (fx+ i 1)))))
    ;; generic case
...
(define (s32vector-fold-right kons knil vec . vecs)
  (if (null? vecs)
    ;; fast path
    (let ((len (s32vector-length vec)))
      (let loop ((r knil) (i (fx- (s32vector-length vec) 1)))
        (if (negative? i)
          r
          (loop (kons r (s32vector-ref vec i)) (fx- i 1)))))
    ;; generic case
...
(define (s32vector-count pred vec . vecs)
  (if (null? vecs)
    ...
    (let ((len (s32vector-length vec)))
      (let loop ((i 0) (r 0))
        (cond
         ((fx= i (s32vector-length vec)) r)
         ((pred (s32vector-ref vec i)) (loop (fx+ i 1) (fx+ r 1)))
         (else (loop (fx+ i 1) r)))))
    ;; generic case
    (let* ((vecs (cons vec vecs))
           (len (apply min (map s32vector-length vecs))))
      (let loop ((i 0) (r 0))
        (cond
         ((fx= i len) r)
         ((apply pred (%s32vectors-ref vecs i)) (loop (fx+ i 1) (fx+ r 1)))
         (else (loop (fx+ i 1) r)))))))
  ...
  (if (null? vecs)
    ...
    (let ((len (s32vector-length vec)))
      (let loop ((i 0) (last #t))
        (cond
         ((fx= i len) last)
         ((pred (s32vector-ref vec i)) => (lambda (r) (loop (fx+ i 1) r)))
         (else #f))))
    ;; generic case
    (let* ((vecs (cons vec vecs))
           (len (apply min (map s32vector-length vecs))))
      (let loop ((i 0) (last #t))
        (cond
         ((fx= i len) last)
         ((apply pred (%s32vectors-ref vecs i)) => (lambda (r) (loop (fx+ i 1) r)))
         (else #f))))))
...
(define (s32vector-fill-some! vec fill start end)
  (unless (fx= start end)
    ...
    (s32vector-fill-some! vec fill (fx+ start 1) end)))
...
(define (reverse-s32vector->list* vec start end)
  (let loop ((i start) (r '()))
    (if (fx= i end)
      r
      (loop (fx+ 1 i) (cons (s32vector-ref vec i) r)))))
  ...
  (let* ((len (length list))
    ...
    (let loop ((i 0) (list list))
      (cond
        ((fx= i len) r)
        (else
          (s32vector-set! r (fx- (fx- len i) 1) (car list))
...
(define (write-s32vector* vec port)
  ...
  (let ((last (fx- (s32vector-length vec) 1)))
    (let loop ((i 0))
      (cond
        ((fx= i last)
  ...
  (let ((len (min 256 (s32vector-length vec))))
    (let loop ((i 0) (r 0))
      (if (fx= i len)
        ...
        (loop (fx+ i 1) (+ r (s32vector-ref vec i)))))))
